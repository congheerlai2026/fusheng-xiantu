...
const UI = {
  ...
  npcImageFor(t) {
    const map = { old_m: "assets/npc_old_m.webp", old_f: "assets/npc_old_f.webp", young_m: "assets/hero_m.webp", young_f: "assets/hero_f.webp" };
    return map[t] || "";
  },
  ...
  renderInventory() {
    ...
    if (inv.length === 0) {
      ...
      inv.forEach(item => {
        ...
        html += `<div class="inv-item"><span class="item-type ${item.type}">${item.type}</span> ${grade}<span class="item-name">${item.name}</span>${showDesc ? `<div class="item-desc">${desc}</div>` : ""}</div>`;
      });
    }
    ...
    if (inv.length === 0 && c.techniques.length === 0) {
      ...
      if (inv.length > 0) {
        invHtml += inv.map(item => {
          ...
          return `<div class="mdrawer-inv-item">${typeTag}${gradeTag}${item.name}</div>`;
        }).join("");
      }
      if (c.techniques.length > 0) {
